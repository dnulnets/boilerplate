...
--
import           Base.Concurrent         (forkChild)
import qualified Base.Log                as Log (Handle (..), Priority (..))
...
defaultConfig = Config "start" 1024
...
loggerThread config chan = do
  ...
  where
    ...
    loggerControlLoop size ix = do
      ...
      case info of
        Left e -> do
          putStrLn $ "Base.Log.FileHandle: Unable to execute the logger thread due to " ++ show e
          return ()
        Right again ->
          if again
          then do
            putStrLn $ "Base.Log.FileHandle: Performing log rotation"
